...
const SignupPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Here, we just navigate to the (simulated) payment page.
    navigate('/payment-simulation', { state: { email: email, name: name } });
  };
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-xl shadow-2xl">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            >
              {isLoading ? <LoadingSpinner size="w-5 h-5" /> : 'Confirmar Assinatura e Pagar (Simulação)'}
            </button>
          </div>
        </form>
